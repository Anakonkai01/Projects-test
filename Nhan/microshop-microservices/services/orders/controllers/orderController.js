...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        // Logic xác định userId cho guest hoặc user đã đăng nhập
        if (req.user) {
            userId = req.user.id;
        } else if (guestEmail) {
            try {
        ...
        if (discountCode) {
            ...
            if (discount && discount.timesUsed < discount.usageLimit) {
                couponDiscountPrice = discount.discountType === 'percentage' ? (itemsPrice * discount.value) / 100 : discount.value;
        ...
        // ĐIỂM SỬA 1: Sử dụng biến 'userId' thay vì 'req.user.id'
        if (pointsToRedeem > 0) {
            try {
                // Chỉ user đăng nhập mới có điểm để sử dụng
                if (!req.user) {
        ...
        const order = await Order.create({
            ...
            user: userId, // Dùng biến userId
            orderStatusHistory: [{ status: 'Pending' }]
        });
        
        if (discountCode) {
            const discount = await Discount.findOne({ code: discountCode });
            discount.ordersApplied.push(order._id);
            await discount.save();
        }

        // Phát sự kiện trừ kho (không thay đổi)
